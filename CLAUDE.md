...
- **The shell opens on a model, with three of §16's eleven views.** `einzel-shell models/reflectron.json` gives a parameter tree with live validation and units on every field, the shared `SessionJournal` with agent and human attribution, and a **3-D viewport drawing trajectory bundles coloured by energy** on Helix Toolkit / DirectX 11. Every action is recorded as the `einzel` command that would reproduce it (Amendment 25).
  ...
  **Two things that were wrong first, both about the camera.** Helix defaults to a perspective camera, and an ion-optics drawing is read for where things are along the axis — the one thing perspective distorts. And `ZoomExtents` fired before layout, so a reflectron's 1.3 m flight ran off the edge: the control had a model but had not been measured, so the fit was to whatever size it had before.
