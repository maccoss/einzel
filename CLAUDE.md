...
- **Stage 4** — `Einzel.Analysis` (Class T figures of merit: arrival-time peak, resolving power both model-free and Gaussian, transmission, TOF focusing-order coefficients) and `Einzel.Library` (`MirrorProfile`, `PlanarMirror`, `MirrorPair`). The memo's mirror pair is modelled from a solved printed-circuit geometry, and **memo §6 item 1 is answered**.
  ...
  | Two-stage, 35% first stage | 767.0 mm | −2.1e-7 | −0.0028 | 320,548 |
...
- **Stage 5, partly done** — `Einzel.Sweeps`: tolerance Monte Carlo, one-at-a-time attribution, sensitivity ranking, and FLD-1 sensitivity fields.

**The FLD-1 spike §23 asked for has been run, and it fails.** Not for the stated reason: the *physics* is linear where §10 says, but the *discretisation* is not. A rasterised electrode makes the discrete operator a staircase function of position. Below one cell a perturbation is invisible — the derivative comes out identically zero and a study would report the parameter as having **no influence**, which is why that is now a hard error rather than a warning. Above one cell the residual is percent-level, four orders over the FLD-2 gate. There is no step size in between. Full table and the routes out (body-fitted mesh, cut-cell, analytic shape derivatives) are in `docs/spec-findings.md`. Voltage channels are unaffected and linearise to 1.5e-14.

**Recommended next, ahead of the optimiser: a cut-cell discretisation.** It fixes the shape-derivative problem *and* the interior-electrode coarsening weakness, which turned out to be the same prerequisite — a campaign over a 513×257 grid crashed the test host because the V-cycle has too few levels on interior-electrode geometry. Building CMA-ES on a solver that cannot differentiate geometry would be building the fast part of a machine whose slow part does not work.
