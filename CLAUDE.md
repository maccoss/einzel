...
- **A packet can cross between the two transport descriptions (SEQ-1's conversion).** §9 says an instrument is a timed state machine of "ordered phases with durations, excitation overrides, **transport mode**, and transition conditions", and SEQ-1 adds that a phase boundary may change it. That is ordinary instrument behaviour rather than an exotic case — ions are collected and thermalised in a gas-filled trap, where the description is a density, then extracted into vacuum and flown, where it is trajectories. The two modes have been peers since REG-1's seam was built and could not hand anything to each other.
  ...
  **Refused rather than patched**: a sequenced model may have one field element. The two coherent readings need work this does not do — either the timeline is the instrument's and every element recompiles at each stage, which is what SEQ-1 needs anyway, or a stage is scoped to its element, which is a different feature from the documented one. No shipped model or template has more than one field element, so this refuses nothing that exists; **latent is not the same as harmless**, since a two-element sequenced model is an ordinary thing to write and the wrong answer was a plausible one. Details in `docs/pressure.md` and `docs/lessons.md`.
