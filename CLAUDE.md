...
- **An analytic element can be bounded, so two instruments share a document (SPEC.md Amendment 32).** An analytic field has no extent, *because a formula does not* — so a quadro-logarithmic potential, which grows as z², puts an enormous field across the trap that injects it. Superposition was never the problem and the sequencer could always express the handover. **And the obvious escape does not exist**: declaring the analyser as *solved* geometry fails because its electrodes are equipotentials of the field they produce, so the profile is transcendental in r (Lambert W) and the 2-D shape vocabulary has no curve a document can name.
  ...
  **What it does not solve is the potential step**, and the limitation is derived rather than guessed. A box is not an equipotential, so a crossing ion gains or loses whatever the inner field held there — and for the fields one most wants to bound that is large: a uniform potential *never decays* (100 V at 50 mm from its own zero) and a quadro-logarithmic one *grows* (7,000 V at 30 mm), so "place the boundary where the field has decayed" has nowhere to point. The step is reported on every bounded element in volts **and** as a fraction of the potential the ion is accelerated through, because a step means nothing alone: 100 V across a 10 V beam is a different instrument and across a 4 kV beam it is 2.5%. So a region is best read as *a statement that ions do not cross that boundary*; if one does, the run says by how much it was wrong.
