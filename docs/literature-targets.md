...
| R against scan rate | R goes as `beta^(-1/4)` | **Recovered asymptotically.** Six ramps from 4 to 128 ms: R = 3.3 / 8.3 / 13.5 / 18.2 / 22.3 / 25.7, the ratio per halving of beta falling 2.52 → 1.63 → 1.35 → 1.23 → 1.15 onto the law's 1.19. Fast ramps fall short because the peak arrives after the exit potential has dropped well below the release value — the same settling lag as the parking runs, converging on the quasi-static release as the ramp slows. **This series stands under solver behaviour version 2**, which PR #37 bumped for its RF-spectrum, cache-keying and period corrections: the 8 ms Ridgeway anchor re-measured on the current build over the same tracked region gives **R = 21.572 against the recorded 21.57**, with sigma 94.757 against 94.76 - so the bump was correctly conservative rather than consequential for this measurement, and none of the figures in this row need restating. Aligning the density grid with the detector plane, which that PR requires, is worth a further **+1.5 per cent** (21.906). `docs/device-templates.md` |
