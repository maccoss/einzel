...
| resolving power | `R = v_g (2L_p/beta)^(1/4) K^(-3/4) sqrt(q / 16 ln2 kT)` — same form as Hill's drift-tube law with the effective path `v_g t_p` in place of the tube length | **Measured across ramps and gas speeds; the shape holds, the level is 0.4-0.7 of the law.** At 50 m/s and 7.5 V/ms R = 11 / 8 / 5 for K × 0.75 / 1.0 / 1.5 (law: 24 / 19 / 14 with `L_p` as parking point to field peak); slowing the ramp to 128 ms takes the reference ion to 25.7 (law 38.8). At Ridgeway's gas profile, imported as velocity and pressure fields, **R = 21.9 at 8 ms, 37.2 at 32 ms and 51.1 at 128 ms** — the last being the highest this model has produced, with every one of 100,000 ions collected, at a ramp duration inside Hernandez's own 100-300 ms. Extrapolating that point to his 300 ms at the same gas gives **63**, so the remaining factor to 100-250 is **1.6 to 4.0**: his faster flow (140 m/s against this profile's 75-130) and an accumulation plateau this tunnel lacks. The 1.6 at the bottom of his band is the ~1.5 this row previously attributed to the width's composition, now bounded by a measurement rather than estimated |
| R against scan rate | R goes as `beta^(-1/4)` | **Recovered asymptotically.** Six ramps from 4 to 128 ms: R = 3.3 / 8.3 / 13.5 / 18.2 / 22.3 / 25.7, the ratio per halving of beta falling 2.52 → 1.63 → 1.35 → 1.23 → 1.15 onto the law's 1.19. Fast ramps fall short because the peak arrives after the exit potential has dropped well below the release value — the same settling lag as the parking runs, converging on the quasi-static release as the ramp slows. **And the law holds over four octaves where the release lag is small, which is the condition it needs.** On Ridgeway's profile the ladder is 21.906 / 37.174 / 51.124 at 8 / 32 / 128 ms, giving **1.303 then 1.173 per doubling** against the law's 1.189 - straddling it and converging from above, the same shape the 50 m/s series shows. **A prediction recorded before the 128 ms run landed shows what the anchor costs**: taken from the 8 ms point the law gives 43.8, which is 14 per cent low, and from the 32 ms point it gives 52.6, which is 2.8 per cent high. The fast anchor imports its own release-lag shortfall into the prediction, which is this row's own explanation for the overshoot at short ramps - and it is corroborated by the elution potential converging as the ramp slows, 60.276 to 67.328 to 70.056 V, extrapolating to a quasi-static release near 74 V. **This series stands under solver behaviour version 2**, which PR #37 bumped for its RF-spectrum, cache-keying and period corrections: the 8 ms Ridgeway anchor re-measured on the current build over the same tracked region gives **R = 21.572 against the recorded 21.57**, with sigma 94.757 against 94.76 - so the bump was correctly conservative rather than consequential for this measurement, and none of the figures in this row need restating. Aligning the density grid with the detector plane, which that PR requires, is worth a further **+1.5 per cent** (21.906). `docs/device-templates.md` |
