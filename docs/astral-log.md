...
## 79. The compact analyzer, and the coin toss it found in the published one

The compact analyzer this project is designing is the reproduced Astral at a fifth of its
size, and until now its only model was `planar-mirror-pair` - one plane of a mirror pair with
no drift, no tilt and no reversal. `compact-astral-3d` is `astral-3d` whole, generated by
`generators/compact-astral-3d.py`: every literal length and duration kept as `<name>FullSize`
with its provenance, one `scale` (0.2), and the working names derived. The cell sizes are
lifted too, so the solve is the full-size discrete problem in smaller units.

**Electrostatic similarity makes the right answer exact**, so it was checked exactly, and it
failed:

| scale | flight ÷ (full-size flight × scale) |
| --- | --- |
| 1.0, 0.5, 0.25, 0.2000001 | 1.00000000 |
| 0.1999999 | 0.99974242 |
| 0.2 | **0.99730632** |

Solve cycle counts were identical at every scale, so it was not the solver. Three of the
foil's shared slice faces (84.375, 225 and 365.625 mm) sat exactly on nodes 80, 128 and 176
of the 4 mm foil mesh, and at 0.2 - which rounds differently from 1.0 - 454 nodes and 1,586 of
244,664 cut links changed hands between adjacent slices. Moving the mesh 1 mm (zPad 151)
restored the ratio to 0.9999999966, which exonerated the faces lying on the domain's own
edges (still on nodes at 151) and convicted the shared ones.

**The 1 mm shift also moved the full-size flight 0.19 percent**, and that turned out to be
the larger finding. With every shared face a tenth of a cell or more from a node, the flight
time against where the foil mesh sits:

| foil mesh | placement, cells | flight, us |
| --- | --- | --- |
| 4 mm | 0.1 / 0.3 / 0.5 / 0.7 | 784.89 / 787.10 / 780.56 / 781.80 |
| 2 mm | 0.1 / 0.3 / 0.5 | 776.44 / 777.08 / 778.18 |

Spread 0.83 then 0.22 percent, a factor of 3.8 for a halved cell; means 783.59 then 777.23,
extrapolating at second order to **775.1 us** against 783.2 by arithmetic. The "786.44 us,
0.4 percent" of section 73 was one draw from the 4 mm spread, and the coin-toss faces were
part of the draw. Two meshes only: a 1 mm foil mesh is 1025 x 65 x 1025, 68 million nodes,
and the solver refuses it - as an internal error rather than a refusal, which is its own defect.

**Shipped:** `meshShiftZ` = 0.3 mm, the grounded boards following the mesh. Every shared face
is 0.0976 of a cell from the nearest node. Flight 784.9047 us, reversal 336.061 mm (from
